...
$sql="SELECT * FROM encuesta_estudiantes_tp WHERE cedula='$cedula_doc' AND cod_esc='$cod_esc' AND cod_mat='$cod_mat' AND sec_mat='$sec_mat' AND lapso='$lapso_cons'";
...
{
    header("Location: consultaencuestaestudiantestp.php?h=1");
}
if(empty($cod_mat))
{
    header("Location: consultaencuestaestudiantestp.php?i=1");
}
if(empty($cod_esc))
{
    header("Location: consultaencuestaestudiantestp.php?j=1");
}
if(empty($sec_mat))
{
    header("Location: consultaencuestaestudiantestp.php?k=1");
}
if(empty($lapso_cons))
{
    header("Location: consultaencuestaestudiantestp.php?l=1");
}
...
$date=explode("-", $fecha);
  ...
  ob_start();
    include(dirname(__FILE__).'/promedioencuestaestudiantestp.php');
    ...
    try
    {
        ...
        $html2pdf->Output('PromedioEncuestaEstudiantesTP.pdf');
    }
