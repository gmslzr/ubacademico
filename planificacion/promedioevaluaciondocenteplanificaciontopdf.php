...
$sql="SELECT * FROM evaluacion_docente_planif WHERE cedula='$cedula_doc' AND cod_esc='$cod_esc' AND cod_mat='$cod_mat' AND sec_mat='$sec_mat' AND lapso='$lapso_cons'";
...
$sql_obs_ev="SELECT * FROM observaciones_planif WHERE id_tipo ='3' AND cedula='$cedula_doc' AND cod_esc='$cod_esc' AND cod_mat='$cod_mat' AND sec_mat='$sec_mat' AND lapso='$lapso_cons'";
...
{
    header("Location: consultaevaluaciondocenteplanificacion.php?h=1");
}
if(empty($cod_mat))
{
    header("Location: consultaevaluaciondocenteplanificacion.php?i=1");
}
if(empty($cod_esc))
{
    header("Location: consultaevaluaciondocenteplanificacion.php?j=1");
}
if(empty($sec_mat))
{
    header("Location: consultaevaluaciondocenteplanificacion.php?k=1");
}
if(empty($lapso_cons))
{
    header("Location: consultaevaluaciondocenteplanificacion.php?l=1");
}
if(!is_numeric($cedula_ev) || strlen($cedula_ev)<6 || strlen($cedula_ev)>8)
{
    header("Location: consultaevaluaciondocenteplanificacion.php?m=1");
}
...
$date=explode("-", $fecha);
  ...
  ob_start();
    include(dirname(__FILE__).'/promedioevaluaciondocenteplanificacion.php');
    ...
    try
    {
        ...
        $html2pdf->Output('PromedioEvaluacionDocentePlanificacion.pdf');
    }
